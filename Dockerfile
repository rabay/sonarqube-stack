...
RUN apt-get update \
    ...
    && apt-get install -y curl gnupg2 unzip procps vim \
...
ENV SONAR_VERSION="6.7.7" \
    ...
    PLGN_FNDBGS_VRSN="3.7.0" \
    PLGN_JCC_VRSN="1.1.0.898" \
    # PLGN_DPNDCY_VRSN="2.0.4" \
    PLGN_PMD_VRSN="3.2.1" \
    PLGN_CHCKSTYL_VRSN="4.32"
    # PLGN_DTKT_VRSN="2.0.0"
...
RUN set -x \
    ...
    && curl -o $SONARQUBE_HOME/extensions/plugins/sonar-findbugs-plugin.jar -fSL https://github.com/spotbugs/sonar-findbugs/releases/download/$PLGN_FNDBGS_VRSN/sonar-findbugs-plugin-$PLGN_FNDBGS_VRSN.jar \
    && curl -o $SONARQUBE_HOME/extensions/plugins/sonar-jacoco-plugin.jar -fSL https://github.com/SonarSource/sonar-jacoco/releases/download/$PLGN_JCC_VRSN/sonar-jacoco-plugin-$PLGN_JCC_VRSN.jar \
    && curl -o $SONARQUBE_HOME/extensions/plugins/sonar-pmd-plugin.jar -fSL https://github.com/jensgerdes/sonar-pmd/releases/download/$PLGN_PMD_VRSN/sonar-pmd-plugin-$PLGN_PMD_VRSN.jar \
    && chown -R sonarqube:sonarqube $SONARQUBE_HOME/extensions/plugins/

#     && curl -o $SONARQUBE_HOME/extensions/plugins/sonar-detekt-plugin.jar -fSL https://github.com/detekt/sonar-kotlin/releases/download/$PLGN_DTKT_VRSN/sonar-detekt-$PLGN_DTKT_VRSN.jar \
#     && curl -o $SONARQUBE_HOME/extensions/plugins/sonar-dependency-check-plugin.jar -fSL https://github.com/dependency-check/dependency-check-sonar-plugin/releases/download/$PLGN_DPNDCY_VRSN/sonar-dependency-check-plugin-$PLGN_DPNDCY_VRSN.jar \
...
#     && curl -o $SONARQUBE_HOME/extensions/plugins/sonar-checkstyle-plugin.jar -fSL https://github.com/checkstyle/sonar-checkstyle/releases/download/$PLGN_CHCKSTYL_VRSN/checkstyle-sonar-plugin-$PLGN_CHCKSTYL_VRSN.jar \
